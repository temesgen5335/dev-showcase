...
├── tests/                    ← `npm test`, offline and dependency-free; auto-discovered
│   ├── reasoning-stripper.test.mts
│   └── smalltalk.test.mts
├── evals/
│   └── chat-behavior.eval.mts  ← `npm run eval`, behavioural regression vs the live model
├── public/
...
└── src/
    ...
    │   └── smalltalk.ts       ← deterministic replies for greetings/thanks/"what can you do?"
    ├── loaders/
...
- **A provider is committed to only once it emits text a visitor would actually see** — at least one non-whitespace character *that survives reasoning-stripping*. The gate used to accept any truthy delta, which two failure modes clear without saying anything:
  - a lone `"\n"`; and
  - a `<think>` opener. `qwen/qwen3.6-27b` on Groq writes its reasoning into the **text** channel, so the gate saw `<`, committed, the stripper then removed the whole block, and the visitor got a blank bubble while a healthy provider sat next in line, unreached. Measured 2026-09-03 with `npm run eval`: 2196 chars produced, none visitor-facing.
  - The stripper is therefore created *before* the gate and fed by it, and the accumulated `opening` is already-stripped — **enqueue it, don't re-push it through the stripper.** Cost of the fix: a model that front-loads a long reasoning block is buffered until it says something real. That's latency on a bad model instead of an empty answer on a good provider.
  - **`qwen/qwen3.6-27b` is deliberately absent from the Groq model list.** It is not a retired ID that 404s cleanly — it answers 200 with nothing, which is strictly worse. The failover chain now recovers from it, but keeping it would only burn a round trip.
...
- **Conversational glue is answered from a table before the model is reached** — `src/lib/smalltalk.ts`, applied in `api/chat.ts` right after body validation. `"hi"` used to get the full decline sentence, because the prompt bounded scope to *questions about Temesgen* and a greeting isn't one. Correct by the letter of the rules, and the worst possible front door.
  - The table covers greetings, thanks, farewells, acknowledgements ("ok", "cool") and "what can you do?" / "who are you?". Responses set `x-llm-provider: rule` and `x-llm-model: smalltalk:<kind>`, so the two paths read the same in a log.
  - **Matching is whole-message and exact after normalisation** — never substring, never keyword. Normalisation lowercases and drops everything that isn't a letter, digit or space, so one entry covers `Hi!`, `hi.`, `HI` and `hi 👋`. This is what keeps `"hi, what did he do at Safaricom?"` out of the table: it normalises to seven words matching no entry and reaches the model as the real question it is.
  - **Precision over recall, deliberately.** A false positive (canned "Anytime!" instead of a real answer) is far worse than a miss, which merely falls through to a model that now handles greetings correctly. Add entries; don't loosen the matcher.
  - **Replies carry no facts** — categories to ask about, never a role, employer, project or metric. Copy there isn't reachable from `portfolio-info.md`, so a hardcoded fact would go stale invisibly. Concrete example questions live once, in `SUGGESTIONS` in `ChatWidget.tsx`, where they render as clickable chips. A unit case enforces this.
  - The widget imports the same classifier to decide when to re-show those chips, so client and server agree on what counts as smalltalk.
...
- System-prompt rules in `buildSystemPrompt()`, in this order — **scope → trust boundary → answering style → examples → fenced data**. Keep that order: instructions must precede the data they govern, or a payload can be read as outranking them. The examples sit last because they illustrate rules already stated.
  - **Greetings are explicitly in scope, stated *before* the decline rule.** Without that, "decline anything that is not a question about Temesgen" swallows "hi" — the single most likely opening message on the site. The fast path above catches the common spellings deterministically, which is *not* a reason to soften this: `"morning! what's he building?"` never reaches the table and has to be handled here.
  - **A few-shot block fixes shape and tone** for eight cases: bare greeting, greeting-plus-question, "what can you do?", thanks, a normal factual question, an in-scope question the CV doesn't cover, a code request using his name as a placeholder, and a prompt-extraction attempt. They are labelled as shape-only, and the factual ones use bracketed placeholders rather than invented answers — a few-shot containing a fake employer is a fake fact the model will happily reuse.
...
- **The transcript is client-supplied, so an `assistant` turn is only a claim.** The endpoint is stateless — the browser posts the whole history every time — so a caller can forge assistant turns to bootstrap a jailbreak. Structural `refine`s reject the cheap versions: the array must **start** with a user turn, **end** with a user turn, contain no **consecutive** assistant turns, and total ≤ `MAX_TOTAL_CHARS` (12,000) across all messages. That narrows the surface; it does not close it (a plausible middle can still be fabricated), which is why the system prompt independently treats history as untrusted.
  ...
  - **`wireMessages()` also windows the history to the server's caps (`MAX_WIRE_MESSAGES` 20, `MAX_WIRE_CHARS` 12,000), or a long chat wedges itself.** The endpoint is stateless and the browser resends the whole transcript every turn, so once a conversation passes ~10 exchanges the untrimmed body exceeds `messages.max(20)` and **every** later send 400s with `Array must contain at most 20 element(s)` — permanently, until the visitor hits `clear`. Same permanent-wedge shape as the empty-placeholder bug. The fix sends a sliding window: the final user turn is always kept, older turns are dropped from the front until the body fits both caps, and a window left starting on an assistant turn is re-trimmed so it still opens with a user turn. Raising the server cap is **not** the fix — it's a context-stuffing control; the client respects it instead.
...
- **Widget affordances that are load-bearing, not decoration:**
  - **The send button becomes `stop` while streaming**, wired to the `AbortController` that `send()` has always created and stored. It was never aborted — dead code — so a long or wrong answer had to be waited out. On abort, an assistant placeholder with no content is dropped so the transcript doesn't keep an empty bubble.
  - **Starter chips re-appear after a purely conversational turn**, not just on an untouched panel. Saying "hi" or "thanks" leaves the visitor exactly where they started — needing something to ask — so that is when the menu should come back. Decided with the same `classifySmalltalk()` the API route uses, so the two can't disagree.
  - **Accessibility:** the transcript is `role="log"` + `aria-live="polite"` with **`aria-busy={streaming}`** — without the busy flag a screen reader interrupts itself on every token. Each bubble carries a visually-hidden "You said:" / "Assistant said:" label, because who-said-what is otherwise conveyed only by `alignSelf`. The input has an `aria-label`, the panel is a labelled `role="dialog"`, and the launcher carries `aria-expanded`/`aria-controls`.
  - **Focus:** opening the panel focuses the input; Escape closes and returns focus to the launcher.
...
- `npm test` — every `tests/*.test.mts`, auto-discovered by a shell loop and run via Node's `--experimental-strip-types` (no test framework, no dependency). Offline and free; safe to run on every change. Add a file and it is picked up.
- `npm run eval` — `evals/chat-behavior.eval.mts`: 27 labelled cases against the **live model**, graded deterministically. Needs a real API key and costs tokens, so it is deliberately *not* part of `npm test` and cannot gate a build. Run it after touching the prompt, the model list, or `lib/smalltalk.ts`.
...
The exceptions are `tests/` and `evals/`, and the reason they exist is the rule for adding more: **test the things whose failure is invisible and whose correct behaviour can't be confirmed by looking at the page.** The reasoning stripper qualifies on both counts — it defends against sampling-dependent model output, so a clean manual run proves nothing, and its bugs live in stream-chunk boundaries you cannot reach from the UI. The smalltalk table qualifies for the opposite reason: its dangerous direction is the *false positive*, and "does a real question still reach the model?" is not a thing you can eyeball across 30 phrasings. Assistant behaviour qualifies most of all — it is sampled, so it can regress on a prompt edit or a model swap with nothing on the page to show it, which is what `npm run eval` is for. Rendering, layout, and content don't qualify; eyeball those. Don't add a test framework for any of it — plain Node type-stripping keeps the dependency count at zero.

**Two things the eval taught that are easy to get wrong in any marker-based grader:**
- **Normalise Unicode before comparing.** gpt-oss writes `10\u202FAcademy` (narrow no-break space) and `Temesgen\u2019s` (curly apostrophe). A plain `includes("10 Academy")` is `false` on that, and the first run reported two confident FAILs against perfectly correct answers. `flatten()` applies NFKC plus explicit quote/dash folding. A grader that fails on invisible characters is worse than no grader — it sends you off to fix a prompt that was never broken.
- **Separate infra failures from behavioural ones.** A 429 says nothing about whether the assistant behaves correctly, so the harness reports `ERROR` apart from `FAIL`. The system prompt is ~6k tokens and Groq's on-demand tier allows 8k/minute, so an unpaced run rate-limits itself into meaningless results — hence the `--delay` default of 8s.
...
- **Don't loosen the smalltalk matcher to substrings or keywords.** Whole-message exact matching is the only thing keeping `"hi, what did he do at Safaricom?"` from being answered with a canned greeting. Add table entries instead.
- **Don't put a fact in a `smalltalk.ts` reply.** Nothing there is reachable from `portfolio-info.md`, so it would go stale with no diff to show it. A unit case fails the build if you do.
- **Don't gate provider commitment on raw model text.** It must be text that survives reasoning-stripping, or a model that emits only `<think>` commits the provider and returns a blank answer with no failover.
- **Don't add the eval to `npm test`.** It needs a live key and spends ~160k prompt tokens per full run; a build that fails because a provider is rate-limited is a build nobody trusts.
...
- **Scope enforcement is still prompt-side and therefore sampled.** `npm run eval` now measures it on every run — 27 cases across greeting / legit / offtopic / inject — so a regression is at least *detectable*, which it wasn't before. It is still not deterministic. A cheap classifier pass would make refusals stable at the cost of latency; deferred, because the eval measures clean and a portfolio bot's blast radius is small.
- **The system prompt is ~6k tokens on every request, against Groq's 8k tokens/minute.** It injects all of `portfolio-info.md` (17k chars) verbatim, so in practice the site serves roughly one model-backed question per minute before Groq 429s and the chain fails over to Gemini. Measured while building the eval, which rate-limited itself immediately. The smalltalk fast path removes the cheapest requests from that budget, but it doesn't touch the real ones. Options, cheapest first: trim the reference text to what a visitor actually asks about; split it and inject only the relevant section; or move to a provider tier with a real TPM allowance. Worth doing before the chatbot sees any traffic.
- **Consider an input-side check for the smalltalk table's blind spot.** It is precision-first by design, so unusual greetings ("ayy", "yooo", a bare emoji) fall through to the model. That is the safe direction and the prompt handles it, but the fallback costs ~6k tokens where the table costs zero.
